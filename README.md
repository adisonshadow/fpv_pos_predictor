...
本项目聚焦低空安全防护核心需求，针对 FPV（第一视角）无人机高机动性、运动模式突变、非线性轨迹显著的特点（如瞬时急加速、高速转弯、悬停 - 冲刺切换等），研发并实现了一套端到端闭环、工业级生产就绪的 FPV 无人机轨迹拦截预测方案。该方案专门解决传统线性预测模型（如卡尔曼滤波）在应对 FPV 非线性运动时精度骤降、跟踪易丢失的行业痛点，能够从 “目标感知 - 轨迹跟踪 - 运动模式识别 - 未来轨迹预测 - 拦截点推荐” 全链路覆盖，为低空拦截系统提供毫秒级响应、高可靠性的决策支撑。
...
#### IMM 3D (`IMMPredictor3D`)

**包含3个模型**: CV (6维) / CA (9维) / Hover (6维)

- **适用场景**: 3D空间高机动跟踪
- **优势**: 比单模型3D预测器精度提升30-50%
